...
export const authService = {
  async checkUser(loginOrEmail: string, password: string): Promise<{status: string, message: string} | {accessToken: string, status: string}>{
   try {
     ...
     console.log(e)
     return {status: 'not Found', message: JSON.stringify(e)};
   }
  }
}
