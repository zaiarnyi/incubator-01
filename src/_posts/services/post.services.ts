...
export class PostServices {
  ...
  async updateStatusCommentToPost(userId: string, postId: string, status: LikeStatus, login: string){
    const like = LikeStatus.Like === status;
    const dislike = LikeStatus.Dislike === status;

    const likePost = await LikeStatusPostCommentsEntity.findOne({postId, userId});
    if(likePost){
      await LikeStatusPostCommentsEntity.updateOne({postId, userId}, {dislike, like, myStatus: status});
      return;
    }
    const createLikeStatus = new LikeStatusPostCommentsEntity();
    createLikeStatus.myStatus = status;
    createLikeStatus.userId = userId;
    createLikeStatus.postId = postId;
    createLikeStatus.like = like;
    createLikeStatus.dislike = dislike;
    createLikeStatus.login = login;

    await createLikeStatus.save();
  }
